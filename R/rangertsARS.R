...
##' rangerts is a fast implementation of random forests (Breiman 2001) or recursive partitioning, particularly suited for high dimensional data.
...
##'
##' For GWAS data consider combining \code{rangerts} with the \code{GenABEL} package.
...
##'
##' See \url{https://github.com/imbs-hl/rangerts} for the development version.
##'
...
##' @param write.forest Save \code{rangerts.forest} object, required for prediction. Set to \code{FALSE} to reduce memory usage if no prediction intended.
...
##' @return Object of class \code{rangerts} with elements
...
##'
##' ## Variable importance
##' rg.iris <- rangerts(Species ~ ., data = iris, importance = "impurity")
##' rg.iris$variable.importance
##'
##' ## Survival forest
##' require(survival)
##' rg.veteran <- rangerts(Surv(time, status) ~ ., data = veteran)
##' plot(rg.veteran$unique.death.times, rg.veteran$survival[1,])
##'
##' ## Alternative interfaces (same results)
##' rangerts(dependent.variable.name = "Species", data = iris)
##' rangerts(y = iris[, 5], x = iris[, -5])
##'
##' \dontrun{
...
##'   }
##' @seealso \code{\link{predict.rangerts}}
##' @useDynLib rangerts, .registration = TRUE
##' @importFrom Rcpp evalCpp
##' @import stats
##' @import utils
##' @importFrom Matrix Matrix
##' @export
rangerts <- function(formula = NULL, data = NULL, num.trees = 500, mtry = NULL,
  ...
  ## Prediction mode always false. Use predict.rangerts() method.
  ...
  if (write.forest) {
    ...
    class(result$forest) <- "rangerts.forest"
